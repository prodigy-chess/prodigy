...
TEST_CASE("search") {
  ...
  const auto simulation_count = GENERATE(0UZ, 256UZ);
  Searcher searcher(1ULL << 30, EvaluationPolicy(), UCTPolicy(1));
  searcher.search(tree, simulation_count);
  REQUIRE(simulation_count ==
          std::accumulate(tree.root().edges().begin(), tree.root().edges().end(), 0UZ,
