 ...
 private:
  ...
  constexpr void visit(const auto& move) const noexcept {
    if (auto& leaf_node_count = static_cast<const Derived&>(*this).leaf_node_count(move); depth_ == Ply{0}) {
      ...
      const auto undo =
          this->template scoped_move<!child_context.side_to_move, child_context.can_en_passant>(node_, move);
      walk<child_context>(node_, SubVisitor(node_, decrement(depth_), leaf_node_count));
    }
  }
